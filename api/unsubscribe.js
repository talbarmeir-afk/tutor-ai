...
module.exports = async (req, res) => {
  ...
  if (!token || !serviceKey) {
    res.status(400).send('<p style="font-family:sans-serif;padding:40px;">Invalid unsubscribe link.</p>');
    return;
  }
  try {
    ...
    if (response.ok && updated.length) {
      res.status(200).send('<p style="font-family:sans-serif;padding:40px;">You’ve been unsubscribed from Claruno progress reports.</p>');
    } else {
      res.status(404).send('<p style="font-family:sans-serif;padding:40px;">That unsubscribe link is no longer valid.</p>');
    }
  } catch (err) {
    res.status(500).send('<p style="font-family:sans-serif;padding:40px;">Something went wrong — try again shortly.</p>');
  }
};
