...
async function computeDigest(userId, serviceKey) {
  const [lessons, entries, profiles] = await Promise.all([
    ...
    // Excludes skipped problems (or ones solved for the student via Q&A)
    // for the same reason the Dashboard does — no genuine attempt happened,
    // so counting them would misrepresent mastery/accuracy in the digest.
    restGet(`history_entries?user_id=eq.${userId}&select=topic,has_mistake,created_at&skipped=eq.false`, serviceKey),
