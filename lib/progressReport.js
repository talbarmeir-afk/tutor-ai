...
function renderDigestHtml(digest, unsubscribeUrl) {
  const name = digest.firstName || 'your student';
  ...
  return `<div style="font-family:-apple-system,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;color:#181816;">
    <h2 style="margin:0 0 4px;">Claruno weekly progress</h2>
    <p style="color:#555;margin:0 0 20px;">A summary of ${name}'s activity this week.</p>
  ...
  for (const [userId, userRecipients] of byUser) {
    ...
    for (const r of userRecipients) {
      try {
        ...
        await sendEmail(r.email, 'Your weekly Claruno progress report', renderDigestHtml(digest, unsubscribeUrl), resendKey, fromAddress);
        sent++;
      } catch (err) {
        failed++;
      }
...
module.exports = { sendAllProgressReports, computeDigest, renderDigestHtml, sendEmail };
