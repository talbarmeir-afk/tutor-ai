...
// Powers auto-zoom-to-content in the lesson camera box: locates the
// actual handwritten/printed math work in a photo, so index.html can zoom
// the DISPLAYED frozen photo into it and crop out dead margin (desk,
// laptop, chair, the paper's own blank edges). Went through three rounds
// of local pixel-heuristic detection first (row density, average
// brightness, contiguous dark-run length) — each fixed one failure mode
// on a real photo and broke on the next (a background object mistaken
// for content, a paper's own shadow/edge mistaken for content, genuine
// light pencil work missed entirely). Telling ink apart from background
// clutter is a visual judgment call, not a pixel-statistics one — the
// model is already reliably good at exactly this kind of call elsewhere
// in this app (orientation, mistake location), so it's worth the extra
// small API round-trip per capture instead of continuing to chase pixel
// thresholds that don't generalize. No thinking, small max_tokens — this
// is a single fast localization call, not a check.
async function detectContentBox(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    // Live testing showed the model correctly enumerating the right NUMBER
    // of items but assigning them badly wrong y-coordinates (e.g. a run of
    // values clustered at 87-128 — past the valid 0-100 range — for lines
    // that were actually spread across most of the frame). With no
    // reasoning budget, a list of several near-identical {x0,y0,x1,y1}
    // objects is exactly the shape a model can pattern-complete instead of
    // actually re-deriving each position. A thinking budget (same lever
    // CHECK_THINKING/CHECK_OUTPUT_CONFIG use for the main check call, at a
    // lighter "medium" effort — this is a much simpler task) gives it room
    // to work out each line's position deliberately instead of guessing a
    // plausible-looking sequence.
    max_tokens: 1200,
    thinking: { type: 'adaptive' },
    output_config: { effort: 'medium' },
    system: 'You are locating everything the student wrote or drew on the page — handwritten or printed math, AND any diagrams, sketches, or figures (e.g. a triangle drawn to go with a Pythagorean theorem problem) — in a photo, so it can be cropped and zoomed in for a student to read more easily. Ignore everything else in the frame — desk, laptop, phone, hands, chair, shadows, the blank margins of the paper itself. Each item\'s box must stay within the physical edges of the page itself — never let it extend past the page onto the desk or background behind it, even if part of a mark or stroke happens to cross that edge. A light blue coordinate grid with percent labels is drawn over the photo purely to help you report positions — the numbers along the top edge are x percentages and the numbers along the left edge are y percentages. The grid is NOT part of the student\'s work — ignore it when looking for content. Before answering, work out each item\'s position one at a time by finding the gridlines directly above/below and left/right of it — do not estimate several items\' positions all at once from a single glance. Respond with ONLY the JSON object requested, no markdown fences, no commentary.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'List EVERY distinct line the student wrote on this page, top to bottom, plus any diagrams/sketches/figures (a drawn triangle, number line, graph, etc) — do not skip any, and do not stop after the first one. For each item, work out its own small bounding box by checking it against the nearest gridlines individually — do not just estimate a sequence of positions. Respond with exactly this JSON shape: {"found": true or false, "items": [{"x0": percent from the left edge to the item\'s left extent, "y0": percent from the top edge to the item\'s top extent, "x1": percent from the left edge to the item\'s right extent, "y1": percent from the top edge to the item\'s bottom extent}, ...one entry per line/diagram]}. Set found to false and items to an empty array if no legible math work or diagram is visible anywhere in the photo.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
  // TEMP DEBUG — rawItems/rawText are surfaced all the way to the client
  // console so the actual model output is visible during live testing,
  // after three different prompt strategies in a row still under-boxed the
  // same test photo. Remove once auto-zoom is confirmed solid.
  if (!textBlock) return { box: null, rawItems: null, rawText: `(no text block, stop_reason: ${data.stop_reason})` };
  try {
    const cleaned = textBlock.text.trim().replace(/^```(json)?/i, '').replace(/```$/, '').trim();
    const parsed = JSON.parse(cleaned);
    if (!parsed.found || !Array.isArray(parsed.items)) return { box: null, rawItems: parsed.items || null, rawText: textBlock.text };
    const items = parsed.items
      .map((item) => ({ x0: Number(item.x0), y0: Number(item.y0), x1: Number(item.x1), y1: Number(item.y1) }))
      .filter((item) => [item.x0, item.y0, item.x1, item.y1].every(Number.isFinite) && item.x1 > item.x0 && item.y1 > item.y0);
    if (!items.length) return { box: null, rawItems: parsed.items, rawText: textBlock.text };
    // Union of every enumerated line/diagram's own box, rather than trusting
    // a single model-reported aggregate box — forces the model to actually
    // account for each line instead of anchoring on whichever single mark
    // (e.g. a bold checkmark) is most visually salient. See fix history for
    // why: two rounds of aggregate-box prompt wording both still latched
    // onto one stray mark and ignored the rest of the page.
    ...
    return {
      box: {
        x0: Math.max(0, Math.min(100, x0)), y0: Math.max(0, Math.min(100, y0)),
        x1: Math.max(0, Math.min(100, x1)), y1: Math.max(0, Math.min(100, y1)),
      },
      rawItems: parsed.items,
      rawText: textBlock.text,
    };
  } catch (err) {
    return { box: null, rawItems: null, rawText: textBlock.text };
  }
}

module.exports = { analyzeImage, askFollowup, checkWatch, hintFromImage, teachSubject, teachFollowup, detectOrientation, detectContentBox, buildWatchPrompt, ANALYSIS_PROMPT, TOPICS };
