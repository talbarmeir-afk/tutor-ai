...
// given lesson.
async function teachSubject(subject, mode, materialsDigest) {
  const testPrep = mode === 'test_prep';
  // Teach-with-materials: a digest of the student's own class/book pages
  // (see extractTeachingMaterials) — teaching must match what school
  // actually taught, not a generic take on the topic. Framing-neutral, so
  // it appends to both the teach and test-prep variants unchanged.
...
Close with one short sentence telling me to write my solution by hand and have Claruno check it.`) + materials,
    }],
  });
  return requireTextBlock(data).text.trim();
}
...
async function extractTeachingMaterials(images) {
  const multi = images.length > 1;
  const content = [
    { type: 'text', text: `Here ${multi ? 'are photos' : 'is a photo'} of pages from a student's class notes or textbook, for a topic they want to be taught.` },
  ];
  images.forEach((img, i) => {
    if (multi) content.push({ type: 'text', text: `Page ${i + 1}:` });
    content.push({ type: 'image', source: { type: 'base64', media_type: img.mediaType || 'image/jpeg', data: img.base64 } });
  });
  content.push({ type: 'text', text: 'Summarize what this material covers and exactly HOW it teaches it, so a tutor can teach the same way: the methods and solution steps it uses, its notation and terminology, its level of difficulty, and the style of its worked examples and exercises. Write a compact plain-text summary — no markdown, no commentary about the photos themselves.' });

  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 1500,
    system: 'You are summarizing a student\'s class/textbook material so a tutor can align their teaching with it. Respond with ONLY the summary itself, plain text.',
    messages: [{ role: 'user', content }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
