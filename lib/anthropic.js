...
function buildWorkContent(base64, mediaType, problemImage, problemText) {
  const content = [];
  if (problemText) {
    content.push({ type: 'text', text: `The problem the student is solving: ${problemText}` });
  }
...
async function extractHomeworkProblems(images) {
...
module.exports = { analyzeImage, askFollowup, checkWatch, hintFromImage, teachSubject, teachFollowup, detectOrientation, extractHomeworkProblems, buildWatchPrompt, ANALYSIS_PROMPT, TOPICS };
