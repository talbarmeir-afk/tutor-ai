...
STEP 0 — First, confirm the photo actually shows handwritten or printed math work somewhere in the frame — equations, numbers, or a worked solution. If it instead shows something else entirely (a blank page, an empty desk, a laptop, a phone, a person, furniture, or anything without legible math content), set no_work_found to true, leave has_mistake as false, and set explanation to one plain sentence describing what the photo actually shows instead of math work — then skip every step below and go straight to the JSON response. A photo with no work in it is NOT the same as a correct solution; never report has_mistake: false just because there's nothing to find wrong with.

If real math work IS visible, continue with the steps below (no_work_found stays false):
