...
Find the handwritten token "${token}" inside the line that reads "${line}". If similar text appears more than once in the crop, use the occurrence inside that line.

Respond with ONLY this JSON: {"found": true or false, "x": number, "y": number} — x and y are the CENTER of that exact token, read off the grid labels (interpolate between lines for precision). If the token is not visible in this crop, set found to false and give your best guess anyway.` },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
