...
async function detectContentBox(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 300,
    system: 'You are locating everything the student wrote or drew on the page — handwritten or printed math, AND any diagrams, sketches, or figures (e.g. a triangle drawn to go with a Pythagorean theorem problem) — in a photo, so it can be cropped and zoomed in for a student to read more easily. Ignore everything else in the frame — desk, laptop, phone, hands, chair, shadows, the blank margins of the paper itself. The box must stay within the physical edges of the page itself — never let it extend past the page onto the desk or background behind it, even if part of a mark or stroke happens to cross that edge. A light blue coordinate grid with percent labels is drawn over the photo purely to help you report positions — the numbers along the top edge are x percentages and the numbers along the left edge are y percentages. The grid is NOT part of the student\'s work — ignore it when looking for content. Read x/y directly off the nearest gridlines instead of estimating proportions by eye. Respond with ONLY the JSON object requested, no markdown fences, no commentary.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'Read through this photo the same way you would to check the work: go through EVERY distinct line the student wrote, top to bottom, plus any diagrams, sketches, or figures (a drawn triangle, number line, graph, etc) — this box must cover the full block of work, not just one line or one mark within it. Then report the bounding box that spans from the TOP edge of the very first thing (whether a line of writing or the top of a diagram) to the BOTTOM edge of the very last thing, and from the LEFTMOST to the RIGHTMOST extent across all of it — not a loose guess at the overall photo, the actual top/bottom/left/right edges themselves, read off the gridlines. Respond with exactly this JSON shape: {"found": true or false, "x0": percent from the left edge to the leftmost extent of the work, "y0": percent from the top edge to the top of the first item, "x1": percent from the left edge to the rightmost extent of the work, "y1": percent from the top edge to the bottom of the last item}. Set found to false if no legible math work or diagram is visible anywhere in the photo.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
  ...
  try {
    ...
    const x0 = Number(parsed.x0), y0 = Number(parsed.y0), x1 = Number(parsed.x1), y1 = Number(parsed.y1);
