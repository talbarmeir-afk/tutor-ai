...
// No extended thinking (overkill and slow for a 4-way classification), but
// enough tokens for a short line of reasoning before the answer — a bare
// forced first-token guess proved unreliable on handwriting specifically.
// Went through two failure modes tuning this: a plain forced guess
// confidently flipped upright work upside-down; a version leaning hard
// toward "answer 0 whenever unsure" swung too far the other way and left
// genuinely sideways/upside-down photos uncorrected. This version instead
// walks through concrete, math-specific cues (rather than a vague
// confidence judgment) and only falls back to 0 when truly nothing is
// legible in any orientation.
async function detectOrientation(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 200,
    system: 'You are judging the rotation needed to make a photo of a handwritten math worksheet upright. Handwriting lacks the strong baseline/x-height cues of printed text, so lean on structural, symbol-level cues instead: an equals sign (=) is two short parallel strokes that are HORIZONTAL when upright; a fraction bar is a HORIZONTAL line with one number above it and one below; digits like 6 and 9, and letters like x and y, have a consistent top and bottom that looks wrong when flipped or turned; a line of writing runs consistently in one direction across the page, not top-to-bottom. Mentally check the image against each of 0, 90, 180, 270: which one makes these cues look normal? Reason briefly (1-2 short sentences) citing a specific cue you actually see (e.g. "the = sign is vertical, so this needs 90° clockwise" or "digits and the fraction bar all read normally already"), then end with a new line containing ONLY: "ANSWER: " followed by 0, 90, 180, or 270. Only answer 0 for "already upright" or for "genuinely illegible in every orientation" — do not use 0 as a default hedge when you can see text but are just not fully certain which way it goes.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'This photo may be rotated. If there is any handwritten or printed text/numbers visible, how many degrees clockwise does the image need to be rotated so that text reads upright (not upside-down, not sideways)? Only answer 0 if it already looks upright or nothing is legible in any orientation.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
  const match = textBlock && textBlock.text.match(/ANSWER:\s*(0|90|180|270)/i);
  return match ? parseInt(match[1], 10) : 0;
}

module.exports = { analyzeImage, askFollowup, checkWatch, hintFromImage, teachSubject, teachFollowup, detectOrientation, buildWatchPrompt, ANALYSIS_PROMPT, TOPICS };
