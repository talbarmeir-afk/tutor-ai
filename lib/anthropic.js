...
STEP 3 — Among the remaining (non-crossed-out) lines only, find the first one with a mathematical error (an incorrect step, wrong sign, dropped term, arithmetic slip, etc), and identify the specific wrong token within that line — the particular number, sign, or term that is actually incorrect (not just the line as a whole). If all remaining lines are correct, note that instead (point at the last written non-crossed-out line).

STEP 4 — Before answering, check: does the line you're about to report as the mistake match any line in your Step 1 list? If so, that's wrong — go back and use the next non-crossed-out line instead. A line from your Step 1 list must never be the line_quote you report.
...
// Powers the "Teach me & train on a subject" mode: a short plain-text
// mini-lesson plus practice problems the student solves by hand and then
// checks through the normal photo/watch flow — no session state, the
// lesson text itself is the whole product.
async function teachSubject(subject) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 1200,
    system: 'You are a friendly, encouraging math teacher. Respond in plain text only — no markdown, no code fences, no asterisks or bullet symbols.',
    messages: [{
      role: 'user',
      content: `A student wants to be taught and then practice this math topic: "${subject}".

First give a short mini-lesson: explain the core idea in a few plain sentences, then walk through one small worked example step by step.

Then present exactly 2 practice problems for them to solve by hand on paper, labelled "Problem 1:" and "Problem 2:", matched to that topic at a typical school level. Do not include the answers.

Close with one short sentence telling them to write their solutions by hand and have Claruno check them.`,
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
  if (!textBlock) throw new Error('The model did not return a text response.');
...
module.exports = { analyzeImage, askFollowup, checkWatch, teachSubject, buildWatchPrompt, ANALYSIS_PROMPT, TOPICS };
