...
async function detectContentBox(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 300,
    system: 'You are locating the handwritten or printed math work in a photo, so it can be cropped and zoomed in for a student to read more easily. Ignore everything else in the frame — desk, laptop, phone, hands, chair, shadows, the blank margins of the paper itself. A light blue coordinate grid with percent labels is drawn over the photo purely to help you report positions — the numbers along the top edge are x percentages and the numbers along the left edge are y percentages. The grid is NOT part of the student\'s work — ignore it when looking for content. Read x/y directly off the nearest gridlines instead of estimating proportions by eye. Respond with ONLY the JSON object requested, no markdown fences, no commentary.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'Read through this photo the same way you would to check the work: go through each distinct line of handwritten/printed math, top to bottom. Then report the bounding box that spans from the TOP edge of the very first line to the BOTTOM edge of the very last line, and from the LEFTMOST to the RIGHTMOST extent of the writing across all of those lines — not a loose guess at the overall photo, the actual top/bottom/left/right edges of the first and last lines themselves, read off the gridlines. Respond with exactly this JSON shape: {"found": true or false, "x0": percent from the left edge to the leftmost extent of the writing, "y0": percent from the top edge to the top of the first line, "x1": percent from the left edge to the rightmost extent of the writing, "y1": percent from the top edge to the bottom of the last line}. Set found to false if no legible math work is visible anywhere in the photo.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
