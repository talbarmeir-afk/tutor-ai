...
async function detectContentBox(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 300,
    system: 'You are locating the handwritten or printed math work in a photo, so it can be cropped and zoomed in for a student to read more easily. Ignore everything else in the frame — desk, laptop, phone, hands, chair, shadows, the blank margins of the paper itself. Respond with ONLY the JSON object requested, no markdown fences, no commentary.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'Find the bounding box of the actual handwritten/printed math content in this photo — the writing itself, not the paper\'s blank margins or anything else in frame. Respond with exactly this JSON shape: {"found": true or false, "x0": percent from the left edge to the left side of the content, "y0": percent from the top edge to the top of the content, "x1": percent from the left edge to the right side of the content, "y1": percent from the top edge to the bottom of the content}. Set found to false if no legible math work is visible anywhere in the photo.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
