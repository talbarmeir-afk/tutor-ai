...
// Powers auto-rotation: run once when the lesson camera starts (only if
// the student hasn't already set a rotation for this lesson), so a camera
// mounted sideways or upside-down is corrected automatically instead of
// requiring a manual tap. No extended thinking (that's overkill and slow
// for a 4-way classification), but enough tokens for a short line of
// reasoning before the answer — a bare forced first-token guess proved
// unreliable on handwriting specifically (e.g. confidently misjudging
// upright work as upside-down), where the usual strong orientation cues
// in printed text (baselines, consistent x-height) are weaker. Biased
// toward 0 on anything short of fairly confident, since a wrong rotation
// actively makes a fine camera angle worse — silence (no rotation) is a
// safer failure than a confident wrong guess.
async function detectOrientation(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 150,
    system: 'You are judging photo orientation from handwriting, which is harder than printed text — cursive-ish digits and sparse marks give weaker cues than a printed baseline. Reason briefly (1-2 short sentences) about which specific cues you actually see (e.g. "the = signs and digit shapes read left-to-right normally" vs "the writing is clearly upside-down"), then end with a new line containing ONLY: "ANSWER: " followed by 0, 90, 180, or 270. If you are not fairly confident, answer 0 — a wrong rotation is worse than no rotation.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'This photo may be rotated. If there is any handwritten or printed text/numbers visible, how many degrees clockwise does the image need to be rotated so that text reads upright (not upside-down, not sideways)? If you cannot tell, or the current orientation already looks fine, answer 0.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
