...
async function detectContentBox(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 300,
    system: 'You are locating everything the student wrote or drew on the page — handwritten or printed math, AND any diagrams, sketches, or figures (e.g. a triangle drawn to go with a Pythagorean theorem problem) — in a photo, so it can be cropped and zoomed in for a student to read more easily. Ignore everything else in the frame — desk, laptop, phone, hands, chair, shadows, the blank margins of the paper itself. A light blue coordinate grid with percent labels is drawn over the photo purely to help you report positions — the numbers along the top edge are x percentages and the numbers along the left edge are y percentages. The grid is NOT part of the student\'s work — ignore it when looking for content. Read x/y directly off the nearest gridlines instead of estimating proportions by eye. Respond with ONLY the JSON object requested, no markdown fences, no commentary.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
