...
Write explanation and fix speaking directly TO the student as "you" (e.g. "you subtracted 37 instead of adding it" or "you're on the right track — try..."), the way a teacher would say it out loud sitting next to them. Never write in the third person — never say "the student" or "they". In every student-facing text (problem_title, explanation, fix), write powers with real superscript characters — x², x³, 10⁵ — never computer caret notation like x^2.
...
function buildWorkContent(base64, mediaType, problemImage, problemText) {
  ...
  if (problemText) {
    ...
    // What to DO about mismatched work diverges by caller — Check flags it
    // as the mistake (analyzeImage), a hint silently ignores it
    // (hintFromImage) — so those rules live with each caller, not here.
    content.push({ type: 'text', text: `The student was assigned this specific problem to solve: ${problemText}

(In the problem text, {numerator}/{denominator} marks a fraction written vertically in the original.)

Judge the handwritten work strictly as an attempt at THIS assigned problem — never as standalone math.` });
  }
  if (problemImage && problemImage.base64) {
    // With an assigned problemText present, the photo is the homework page
    // the problem came from — its value is the figure/diagram the text
    // can't carry (the page may hold other problems too). Without one,
    // it's the classic single printed-problem shot.
    content.push({ type: 'text', text: problemText
      ? 'The next image is the homework page the assigned problem comes from — use it to see any figure, diagram, or sketch the assigned problem refers to. The page may contain other problems as well; only the assigned problem stated above matters.'
      : 'The next image is the PRINTED PROBLEM the student is solving, photographed from a book or worksheet — use it as the actual problem statement and target answer instead of inferring the problem from the handwriting alone.' });
  ...
  const data = await callAnthropic({
    ...
    messages: [{
      ...
      content: [
        ...
        // Check-only mismatch rule (hints deliberately do the opposite —
        // see hintFromImage): a checked solution for the wrong problem
        // must fail, however internally correct its steps are.
        ...(problemText ? [{ type: 'text', text: 'Before anything else, confirm the work actually addresses the assigned problem stated above. If the writing clearly solves a DIFFERENT problem instead (a different equation, different variables or numbers — most likely an old page or leftover work caught in the shot), do NOT treat it as a mistake in that other work: it means the student did not actually attempt the assigned problem. In that case add "wrong_problem": true to the JSON response (false or omitted otherwise), set has_mistake to false, leave line_quote, wrong_token and fix as empty strings and mistake_line_index as 0, and write explanation as: ONE plain opening sentence telling the student it looks like they didn\'t solve the problem they were given, then a short, complete step-by-step walkthrough of how to solve the ASSIGNED problem, ending with its final answer. This problem is being recorded as skipped, so revealing the full solution is the point — teach it clearly. Use the assigned problem itself for problem_title.' }] : []),
...
async function hintFromImage(base64, mediaType, problemImage, problemText) {
  const data = await callAnthropic({
    ...
    thinking: CHECK_THINKING,
    output_config: CHECK_OUTPUT_CONFIG,
    system: 'You are a patient, encouraging math teacher sitting next to a student. Respond in plain text only — no markdown, no JSON, no code fences. Write powers with real superscript characters — x², x³, 10⁵ — never computer caret notation like x^2.',
    messages: [{
      role: 'user',
      content: [
        ...buildWorkContent(base64, mediaType, problemImage, problemText),
...
- If my work is already COMPLETE and correct — I've reached the right final answer — there is nothing to hint: just tell me warmly that it looks right. Do not invent a next step, do not nitpick.
- A skipped intermediate step is fine and never worth a hint: if I jumped from one correct line straight to another correct line without writing the in-between work (e.g. x² = 25 straight to x = 5), that's correct flow — never point it out or ask me to double-check the operation I "applied" in between.
- If my work so far is correct but unfinished, hint at what to do next — name the operation or idea, not the result (say "try moving the constant term to the other side", never "subtract 37 to get -17").
...
- If I haven't actually written any solution work yet — a blank page, or nothing beyond the problem itself copied out — do NOT mention that, don't tell me to write something and photograph it again, and don't describe what's in the photo. Just hint at how to begin the problem I was given: name the right first step or the key idea, as if I'd asked "where do I start?".
- If I was assigned a specific problem and the writing in the photo is clearly work for a DIFFERENT problem, ignore that writing completely — it's almost certainly an old page or leftover work caught in the shot by mistake. Don't mention it, don't point out any mismatch — just hint at how to begin the assigned problem, as if I hadn't started yet.
- Never state the final answer and never write out the next line for me.

HARD RULE: your reply talks ONLY about the math. Never describe or mention the photo or what it shows — not a blank page, not objects on the desk, not missing or absent work, nothing about the picture at all, under any circumstances. Start your reply directly with the math help itself.

Answer in 1-3 short, encouraging sentences spoken directly to me.

After your answer, on its own new line, write exactly "SOLVED: true" if my work is complete and reaches the correct final answer, or "SOLVED: false" otherwise.` },
      ],
    }],
  });
  // Same marker pattern as askFollowup's REVEALED_SOLUTION: the client
  // uses solved=true to run a real Check automatically instead of showing
  // a hint — a student hinting on finished, correct work meant to check it.
  const raw = requireTextBlock(data).text.trim();
  const match = raw.match(/\n?\s*SOLVED:\s*(true|false)\s*$/i);
  return {
    text: match ? raw.slice(0, match.index).trim() : raw,
    solved: !!(match && match[1].toLowerCase() === 'true'),
  };
}
...
(In the problem text, {numerator}/{denominator} marks a fraction written vertically in the original — don't repeat that notation back to me, just talk about the fraction naturally.)
...
const TEACH_SYSTEM = `You are a friendly, encouraging math teacher writing by hand on a sheet of paper for one student. Respond in plain text only — no markdown, no code fences, no asterisks, no bullet symbols. Write the way you would on paper: lines of at most about 70 characters (insert your own line breaks), one idea or one step per line, with a blank line between separate ideas. Write powers the way they look on paper, with real superscript characters — x², x³, 10⁵, xⁿ — never computer caret notation like x^2.

One exception to plain text: when a simple picture would genuinely help the explanation — and for most topics it does: a pizza or chocolate bar for fractions, a labeled triangle for geometry, a number line for negatives, a simple graph for functions — include exactly ONE friendly illustration at the very END of your response, between a line containing exactly <<<FIGURE>>> and a line containing exactly <<<END FIGURE>>>: a single self-contained svg element whose opening tag is exactly <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300"> — the xmlns attribute is required or the drawing will not display. Build it only from basic shapes, lines, paths, and text labels, in bright kid-friendly colors, directly illustrating the idea or example you just taught — or, when you are assigning a practice problem, the problem's own setup (e.g. the triangle with its GIVEN sides and angles labeled). A problem figure must show only the given data, never anything that reveals or hints at the answer. Write any & inside text labels as &amp;. No scripts, no external references, nothing outside the svg tags. The handwriting rules above apply only to the text part, never to the figure.

Detours for prerequisite gaps: if the student's question reveals they are missing PREREQUISITE knowledge — something from an earlier topic they need in order to follow this one (say, fractions inside a trigonometry lesson) — do not answer it in passing. Take a deliberate detour, like a real tutor would: make your reply's very FIRST line exactly DETOUR: <2-4 word name of the missing piece> (a machine marker — the app displays the reply as a clearly labeled detour), then teach that missing piece briefly from the ground up with one tiny example, and end the reply with a single line that bridges back to the main topic (e.g. "Now, back to our triangle…"). Reserve detours for genuine prerequisite gaps; ordinary clarifying questions about the current topic get ordinary answers with no marker.

The figure must be GEOMETRICALLY ACCURATE — a student learns the relationship from the picture, so a picture that contradicts the lesson actively teaches the wrong thing. Before writing the svg, compute the actual coordinates so that every relationship you teach or state holds exactly in the drawing: a shape inscribed in a circle has ALL its vertices exactly on the circle (each corner at distance r from the center — for a rectangle of width w and height h centered on the circle's center, (w/2)^2+(h/2)^2 must equal r^2); tangent lines touch at exactly one point; a diameter passes through the center with both endpoints on the circle; right angles look like right angles; and lengths you label as equal or in a given ratio are drawn equal or in that ratio. After computing, double-check each stated contact/tangency/proportion against your numbers and fix the coordinates BEFORE emitting the svg — never ship a figure where something described as touching floats apart.`;
...
// given lesson.
async function teachSubject(subject, mode, materialsDigest, testWhen) {
  const testPrep = mode === 'test_prep';
  // Teach-with-materials: a digest of the student's own class/book pages
  // (or, in test prep, of past tests / example questions — see
  // extractTeachingMaterials' kind param) — teaching must match what
  // school actually taught / what the test actually looks like, not a
  // generic take on the topic.
  const materials = materialsDigest
    ? (testPrep
      ? `

The student's past tests / example questions for this test are summarized below — make every practice problem feel like it could actually appear on this test: same topics, same question formats and phrasings, same difficulty. Summary: ${materialsDigest}`
      : `

The student's class/book material for this topic is summarized below — align your teaching with it: use the same methods, notation, and terminology, match its difficulty, and make practice problems feel like its exercises. Material summary: ${materialsDigest}`)
    : '';
  // When the test is (free text — "Tuesday", "in 4 days") drives pacing:
  // a test tomorrow needs high-yield reminders and realistic practice, a
  // test in two weeks has room to build understanding properly.
  const pacing = testPrep && testWhen
    ? `

The test is: ${testWhen}. Pace the review for that — if it's very soon, focus on the highest-yield reminders and realistic practice; if there's more time, build understanding step by step.`
    : '';
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 2600,
    ...
    messages: [{
      role: 'user',
      content: (testPrep ? `The student has a test coming up on: "${subject}". Help them review for it.

Give a short review: the core ideas they should remember for this, in a few plain sentences, plus a quick worked-example reminder if it helps — framed as review/practice for a test, not a first-time introduction.

Do NOT give a practice problem yet — first make sure they understand the review. Close with one short sentence inviting them to ask about anything that isn't clear, and to say when they're ready for a practice problem.` : `Teach me this math topic: "${subject}".

Give a short mini-lesson: explain the core idea in a few plain sentences, then walk through one small worked example step by step, the way you'd scribble it on paper while sitting next to me.

Do NOT give me a practice problem yet — first make sure I understand. Close with one short sentence inviting me to ask about anything that isn't clear, and to say when I'm ready for a practice problem.`) + pacing + materials,
    }],
  });
  return requireTextBlock(data).text.trim();
}
...
async function extractHomeworkProblems(images, note) {
  ...
  const content = [
    { type: 'text', text: `Here ${multi ? 'are photos' : 'is a photo'} of a student's homework page${multi ? 's' : ''} — printed or handwritten problems the student has NOT yet solved. Read through ${multi ? 'each photo' : 'it'} and transcribe every distinct problem as its own plain-text item, in reading order (top to bottom, left to right, across all photos in the order given). Skip anything that isn't itself a problem to solve — page headers, instructions, already-worked examples, page numbers. Notation: when a fraction appears VERTICALLY in the source (numerator stacked over a horizontal bar over the denominator), transcribe it as {numerator}/{denominator} — curly braces around each part — so it can be displayed stacked again. This is decided ONLY by how the source draws that fraction, never by how simple it is: a stacked 2 over 3 becomes {2}/{3} exactly like {4x+25}/{5x}. Only a fraction the source itself writes on one line with a slash (like 2/3) stays plain text. Never use curly braces for anything else. Likewise write powers with real superscript characters exactly as they appear in the source — x², x³, 10⁵ — never computer caret notation like x^2.` },
  ];
  ...
  // Per-problem figure flag + source-photo index, never crop coordinates:
  // "which photo is it on" and "does it have a figure" are judgments a
  // vision model makes reliably, while exact bounding boxes are not (see
  // this app's auto-zoom history). Figure problems get presented alongside
  // their whole source page instead of a model-cropped snippet.
  content.push({ type: 'text', text: 'Respond with ONLY this JSON shape, no markdown fences, no commentary: {"problems": [{"text": "full text of the problem exactly as written/printed", "photo": which photo it appears in (1-based number), "has_figure": true if the problem includes or refers to a figure, diagram, sketch, graph, or geometric drawing on the page, else false}, ...]}. One entry per problem, in reading order. Empty array if no legible unsolved problems are visible.' });
  ...
  try {
    ...
    return parsed.problems
      .map((p) => {
        // Tolerate the old plain-string shape too, in case the model
        // answers that way despite the schema.
        if (typeof p === 'string') return { text: p.trim(), photoIndex: 1, hasFigure: false };
        if (!p || typeof p.text !== 'string') return null;
        const idx = Math.round(Number(p.photo));
        return {
          text: p.text.trim(),
          photoIndex: Number.isFinite(idx) && idx >= 1 && idx <= images.length ? idx : 1,
          hasFigure: !!p.has_figure,
        };
      })
      .filter((p) => p && p.text);
  } catch (err) {
...
// Powers teach-with-materials: reads photos of the student's class notes /
// textbook pages ONCE and produces a text digest that then seeds the whole
// (text-only) teach conversation — the digest travels inside the seed
// message, so every follow-up stays aligned with the material at zero
// further image cost. Same read-and-describe task shape as
// extractHomeworkProblems; plain text out, no JSON structure needed.
// kind 'past_tests' switches the framing for test prep: the photos are
// past tests / example questions, so the digest describes what the TEST
// looks like (topics, question formats, difficulty) rather than how the
// class material teaches — a tutor prepping for a test needs to produce
// realistic practice questions, not mirror a textbook's pedagogy.
async function extractTeachingMaterials(images, kind) {
  const pastTests = kind === 'past_tests';
  const multi = images.length > 1;
  const content = [
    { type: 'text', text: pastTests
      ? `Here ${multi ? 'are photos' : 'is a photo'} of past tests or example questions for a test a student is preparing for.`
      : `Here ${multi ? 'are photos' : 'is a photo'} of pages from a student's class notes or textbook, for a topic they want to be taught.` },
  ];
  images.forEach((img, i) => {
    if (multi) content.push({ type: 'text', text: `Page ${i + 1}:` });
    content.push({ type: 'image', source: { type: 'base64', media_type: img.mediaType || 'image/jpeg', data: img.base64 } });
  });
  content.push({ type: 'text', text: pastTests
    ? 'Summarize what this test covers and what its questions look like, so a tutor can produce realistic practice questions in the same style: the topics tested, the question formats and phrasings, the level of difficulty, and how many steps the problems typically take. Write a compact plain-text summary — no markdown, no commentary about the photos themselves.'
    : 'Summarize what this material covers and exactly HOW it teaches it, so a tutor can teach the same way: the methods and solution steps it uses, its notation and terminology, its level of difficulty, and the style of its worked examples and exercises. Write a compact plain-text summary — no markdown, no commentary about the photos themselves.' });

  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 1500,
    system: pastTests
      ? 'You are summarizing past tests / example questions so a tutor can prep a student with realistic practice in the same style. Respond with ONLY the summary itself, plain text.'
      : 'You are summarizing a student\'s class/textbook material so a tutor can align their teaching with it. Respond with ONLY the summary itself, plain text.',
    messages: [{ role: 'user', content }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
...
module.exports = { analyzeImage, askFollowup, checkWatch, hintFromImage, hintFromProblemText, teachSubject, teachFollowup, detectOrientation, extractHomeworkProblems, extractTeachingMaterials, buildWatchPrompt, ANALYSIS_PROMPT, TOPICS };
