...
// Second-pass pointer refinement: the client zooms into the checker's
// first x/y guess, overlays a fine grid labeled in FULL-image percent
// coordinates, and this call re-reads the wrong token's exact position off
// that grid. Locating one token in a zoomed crop is a far easier vision
// task than pinpointing it on a whole page, which is where single-pass
// placement kept missing.
async function locateToken(base64, mediaType, token, line) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 300,
    system: 'You locate handwriting in photos precisely. Respond with ONLY the JSON object requested — no markdown fences, no commentary.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: `This is a zoomed-in crop from a larger photo of handwritten math work. The blue grid lines and numbers on it are percent coordinates of the ORIGINAL FULL photo (not of this crop): numbers along the top are x percents, numbers along the left edge are y percents.

Find the handwritten line that reads "${line}". If similar text appears more than once in the crop, use the full line that matches this text.

Respond with ONLY this JSON:
{"found": true or false, "line_x_min": number, "line_x_max": number, "line_y_min": number, "line_y_max": number, "token_x": number, "token_y": number}

All numbers are percent coordinates of the ORIGINAL FULL photo, read directly off the grid labels (interpolate between lines for precision). line_x_min/line_x_max/line_y_min/line_y_max is the tight bounding box around that ENTIRE handwritten line — read each edge from the nearest gridline. token_x/token_y is the center of the specific token "${token}" within that line. If the line is not visible in this crop, set found to false.` },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
...
module.exports = { analyzeImage, askFollowup, checkWatch, locateToken, teachSubject, teachFollowup, buildWatchPrompt, ANALYSIS_PROMPT, TOPICS };
