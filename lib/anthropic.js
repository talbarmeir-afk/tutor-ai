...
Find the first line where the student makes a mathematical error (an incorrect step, wrong sign, dropped term, arithmetic slip, etc). If the whole solution is correct, note that instead (point at the last written line).

Respond with ONLY this JSON object, no markdown fences, no extra text:
{"has_mistake": true or false, "line_quote": "short exact snippet of text/expression from that line", "explanation": "1-2 plain sentences on what's wrong", "fix": "one sentence on the correct step, or empty string if there's no mistake", "y": percent from the top edge of the photo to the vertical center of that specific line, "x": percent from the left edge of the photo to roughly where that line's text sits horizontally}

For x and y: look directly at the pixel position of that one line in the photo. Do not estimate it from the line's position in a numbered sequence or assume lines are evenly spaced — handwritten spacing is often uneven, so judge this line's position independently of the others.`;
