...
// requiring a manual tap. Deliberately no extended thinking and a tiny
// max_tokens — this only needs to be fast, not deeply reasoned, since it
// blocks the student from starting on the live view.
async function detectOrientation(base64, mediaType) {
  const data = await callAnthropic({
    model: MODEL,
    max_tokens: 10,
    system: 'Respond with ONLY a single number: 0, 90, 180, or 270. No words, no punctuation.',
    messages: [{
      role: 'user',
      content: [
        { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64 } },
        { type: 'text', text: 'This photo may be rotated. If there is any handwritten or printed text visible, how many degrees clockwise does the image need to be rotated so that text reads upright (not upside-down, not sideways)? If you cannot tell (e.g. no visible text), answer 0.' },
      ],
    }],
  });
  const textBlock = (data.content || []).find((b) => b.type === 'text');
  const match = textBlock && textBlock.text.match(/90|180|270|0/);
